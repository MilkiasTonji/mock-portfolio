...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
    return (
        <li className='col-span-1 w-full p-4 bg-light dark:bg-dark border border-solid border-dark dark:border-light rounded-2xl relative'>
            <div className={`absolute top-0 -right-2  bg-dark dark:bg-light rounded-br-3xl -z-10 w-[101%] h-[102%] rounded-2xl`} />
...
const Article = ({ img, title, date, link }) => {
    ...
    first:mt-0 border border-solid border-dark border-r-4 border-b-4 dark:border-light sm:flex-col'>
            <MovingImg
                ...
                link={link}
            />
            <span className='text-primary font-semibold pl-4 dark:text-primaryDark sm:self-start sm:pl-0 sm:mt-2 xs:text-sm'>{date}</span>
        </motion.li>
    )
}
